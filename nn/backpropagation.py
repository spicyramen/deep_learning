...
def initialize_network(n_inputs, n_hidden, n_outputs):
    ...
    print 'Number of inputs: %d Layers: %d, Outputs: %d' % (n_inputs, n_hidden, n_outputs)
    network = list()
...
def activate(weights, inputs):
    ...
    for i in range(len(weights) - 1):
        activation += weights[i] * inputs[i]  # Wi1 * Xi1 +  Wi2 * Xi2 + ... + WiN * Xin
    return activation


def transfer(activation):
    """Transfer neuron activation."""
    return 1.0 / (1.0 + math.exp(-activation))  # Activation function. 1/ 1 + e^-net h1
...
def transfer_derivative(output):
    """ Calculate the derivative of an neuron output.
        ...
        is the output multiplied by 1 minus the output.
    """
...
for input in dataset:
    ...
    print('Expected=%d, Got=%d' % (input[-1], prediction))
